...
def run_loop(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if verbose:
            print(f"Train: {train_passed}/{train_total} passed ({eval_elapsed:.1f}s)", file=sys.stderr)
            if test_summary:
                print(f"Test:  {test_summary['passed']}/{test_summary['total']} passed", file=sys.stderr)
            for r in train_result_list:
                status = "PASS" if r["pass"] else "FAIL"
                print(f"  [{status}] {r['triggers']}/{r['runs']} expected={r['should_trigger']}: {r['query'][:60]}", file=sys.stderr)

        if train_summary["failed"] == 0:
            exit_reason = f"all_passed (iteration {iteration})"
            if verbose:
                print(f"All train queries passed on iteration {iteration}!", file=sys.stderr)
            break
