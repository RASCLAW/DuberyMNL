...
- **2026-05-28** **Schedule tab merged view + CC perf (session 182).** Replaced the 3 top-tab Schedule sub-panels with one unified 3-column row: **Compose | tabbed AI Suggest·Calendar | Preview**. AI Suggest dropped its left sidebar for an orange image-aware banner; caption "Quick asks" became 6 collapsible caption-angle chips (Casual / Bold hook / Outdoorsy / Founder voice / Holiday angle / Social proof) in a default-closed `<details>`. All 3 columns capped at `calc(100vh - 160px)` with internal scroll so the chat never pushes past the viewport. Single-image FB preview now matches source aspect (1x1 renders 1x1; was force-cropped to 4/5). Content Gen history thumbnails swapped from full-res `/api/images/` to `/api/thumb/?w=240` + lazy-load (~100x lighter). Silenced the hourly CMD-flash by switching the `DuberyMNL_FeedScheduler` task from `python.exe` to `pythonw.exe` (task kept -- it's the retry/verify/fallback layer behind Meta-native handoff). CC restart also loaded the session-178 Experiment Mode routes. Holiday banner + chip-state persistence still pending. See `project_schedule_merged_view.md`.
- **2026-05-27** **Live ad consolidation + daily TG digest (session 180).** Collapsed 2 adsets -> 1 via Meta API: paused Bespoke UGC adset entirely, kept Brand Graphics (2x more efficient per session 176 analysis), reactivated BRAND-V3-SPLIT, cloned 5 unique creatives from Bespoke UGC into Brand Graphics. End state: **11 active ads at P100/day** (budget bumped from P70 -- inside 30%/week safe scaling). Image-hash dedupe of the keep list cut clones from 15 -> 5 by detecting same image_hash + same copy text. New tool: `tools/meta_ads/daily_digest.py` -- daily 9 AM PHT TG ping with yesterday's ad performance + 7-day rolling ROAS (cash + Pixel side-by-side), top-CPL ad, biggest-spend ad. Archives each digest to `.tmp/daily_digest/YYYY-MM-DD.md`. Scheduler registration script at `tools/meta_ads/install_daily_digest_task.ps1`. RA also called step (l) kraft hero CDN upload + (m) new model shots both as DEFERRED -- the chatbot image bank (44 picks, 11 variants) is quality enough. **Priority #1 original recovery checklist (h)(i)(l)(m)(o) now all closed** -- system-side complete. Remaining gate to RAS Creative: portfolio case study page + first paying client signed. See `project_ad_consolidation_2026_05_27.md`, `project_daily_ad_digest_tool.md`, `feedback_consolidate_inspect_creatives_first.md`.
