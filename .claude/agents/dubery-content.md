...
- `image_gen/` -- Vertex / Gemini image generation
...
Step 3: For image generation, always validate the prompt before sending to Vertex (paid API).
...
- If the Vertex call fails, stop -- don't retry without checking quota/billing with RA
